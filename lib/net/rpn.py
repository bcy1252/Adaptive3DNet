...
class RPN(nn.Module):
    ...
    def forward(self, input_data):
        ...
        if cfg.AF_FUSION.ENABLED:
            ...
            # torch_out = torch.onnx.export(self.backbone_net, 
            #                               args = (pts_input, img_input, xy_input), 
            #                               f = "CYNet21028.onnx",
            #                               opset_version=10,
            #                               input_names=["pts_input", "img_input", "xy_input"],
            #                               output_names=["backbone_xyz", "backbone_features"])
            # with SummaryWriter(comment='backbone_net') as w:
            #     w.add_graph(self.backbone_net, (pts_input, img_input, xy_input), True)
        ...
        # with SummaryWriter(comment='rpn_cls_layer') as w:
        #     w.add_graph(self.rpn_cls_layer, (backbone_features), True)
        # with SummaryWriter(comment='rpn_reg_layer') as w:
        #     w.add_graph(self.rpn_reg_layer, (backbone_features), True)
