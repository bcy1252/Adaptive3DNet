...
class Pointnet2MSG(nn.Module):
        ...
        for k in range(cfg.RPN.FP_MLPS.__len__()):
            ...
            if k == 0:
                self.FP_modules.append(
                        PointnetFPModule(mlp = [259,128,128])
                ) 
                continue               
            self.FP_modules.append(
    ...
    def forward(self, pointcloud: torch.cuda.FloatTensor, image=None, xy=None):
        ...
        if cfg.AF_FUSION.ENABLED:
            ...
            l_xy_cor = [xy]
            img = [image]
            rgb_xyz_concat = Feature_Gather(image,xy) #single branch
            # a = self.SA_modules[0](l_xyz[0], rgb_xyz_concat)
            # a = self.SA_modules[0](l_xyz[0], l_features[rgb_xyz_concat])
            l_features = [rgb_xyz_concat]
        for i in range(len(self.SA_modules)):
            ...
            #     att = self.AF_Blocks[i](fused_features)
            #     l_att_features = att * li_features
            #     I_att_features = (1 - att) * img_gather_feature
            #     # fused_att_features = torch.cat([l_att_features, I_att_features], 1)
            #     final_features = self.Fusion_Conv[i](l_att_features, I_att_features)
