...
def eval_one_epoch_joint(model, dataloader, epoch_id, result_dir, logger):
    ...
    for data in dataloader:
        ...
        # torch.cuda.synchronize()
        # ender.record()
        # curtime = starter.elapsed_time(ender)
        ...
        # distance thresh
        # dis_thresh = 40
        # rcnn_cls = rcnn_cls[pred_boxes3d[:, :, 2] > dis_thresh].unsqueeze(0)
        # pred_boxes3d  = pred_boxes3d[pred_boxes3d[:, :, 2] > dis_thresh].unsqueeze(0)  
        ...
        if not args.test:
            ...
            for k in range(batch_size):
                ...
                # distance thresh
                # dis_thresh = 60
                # cur_gt_boxes3d = cur_gt_boxes3d[cur_gt_boxes3d[:,2] > dis_thresh]
